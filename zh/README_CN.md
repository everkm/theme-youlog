...
| 参数 | 说明 |
|------|------|
| `hide_prev_next: true` | 隐藏列表项中的上下篇导航链接 |
| `show_summary: true` | 在标题下方展示摘要（有摘要时） |

### dcard `items` 参数

| 参数 | 说明 |
|------|------|
| `cols` | 最大列数；按列表容器宽度自动增减列数，超长标题单行省略 |
| `more` | Obsidian Wikilink，解析成功后在列表末尾追加一项；如 `[[archives]]`、`[[blog/index\|查看全部]]`；解析失败则不显示 |
