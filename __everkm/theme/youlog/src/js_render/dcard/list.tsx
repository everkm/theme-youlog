    ...
    <>
      ...
      <Show when={pageCount() > 1}>
        <div class="space-x-6 flex !mt-10 text-normal items-center justify-center">
          ...
          {/* 跳转分页 */}
          <div
            x-data={`{ currentPage: ${pageNo()}, totalPages: ${pageCount()}, baseUrl: '${pagePathBase()}', pageQuery: '${ctx().env_is_preview ? "" : ""}', goToPage() { const page = parseInt(this.currentPage); if (page >= 1 && page <= this.totalPages) { let url; if (page === 1) { url = this.baseUrl + '.html'; } else { url = this.baseUrl + '.p' + page + '.html'; } window.dispatchEvent(new CustomEvent('page-navigate', { detail: { url } })); } } }`}
          ...
          >
            ...
            >
              ...
              <For each={Array.from({ length: pageCount() }, (_, i) => i + 1)}>
                {(p) => (
                  <option value={p} selected={p === pageNo()}>{`${p} / ${pageCount()}`}</option>
                )}
              </For>
            </select>
          </div>
